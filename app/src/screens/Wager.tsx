    ...
    <>
          ...
          <div className="wlist">
            {WAGERS.map((x) => (
              <button key={x.id} className={'wopt' + (pick === x.id ? ' on' : '')} onClick={() => setPick(x.id)}>
                <span className="we">{x.emoji}</span><span className="wl">{x.label}</span><span className="wp">{x.payout}</span>
              </button>
            ))}
          </div>
        </div>
      ...
      <div className="paybar">
        ...
        <button className="pay" onClick={place} disabled={busy}>
          {busy ? 'Matching…' : <>Place {stake} USD₮ wager <span className="f">· {w.emoji} {w.label} · gasless ⚡</span></>}
        </button>
      </div>
    </>
  )
